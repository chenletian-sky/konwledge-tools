...
import { ClusteringPlotDataType } from '../../types/propsTypes';

import { PATH } from '../../types/actionTypes';
import axios, { AxiosResponse } from 'axios';
import { changeClusteringPlotClass, changeMenuSelect, updateMarkTextData } from '../../action';
import { Slider } from 'antd';

import { connect } from 'react-redux';

// import {changeClusteringPlotClass} 
...
interface ClusteringPlotProps {
    ...
    changeClusteringPlotClass:typeof changeClusteringPlotClass
    history:any
}
...
    }
    ...
    draw_circle = ()=>{
        ...
        var xx = function(id:string){
                scatter_data[id].nodes.forEach((item)=>{
            ...
            .on("click",(e,d)=>{
                ...
                axios.get(`${PATH}/get_scatterData`,{withCredentials:true}).then((res:AxiosResponse<any,any>) => {
                    if(res.data.status === 200 ){
                        console.log(res.data.data[d['class_id']]['nodes'])
                        let selectData = res.data.data[d['class_id']]['nodes']
                        let afterData = selectData.filter((_obj: any,index: any)=>{
                            return index < Math.round(  selectData.length * this.state.percent *0.01  )
                        })
                        // let afterData = selectData.map((obj: any,index: any)=>{
                        //     if(index < Math.round(  selectData.length * this.state.percent * 0.01  ))
                        //         return obj
                        // })
                        afterData = afterData.map((obj: any,index: any) => {
                        ...
                        axios.get(`${PATH}/get_texts`,{withCredentials:true}).then((res:AxiosResponse<any,any>) => {
                            if(res.data.status === 200 ){
                                ...
                                textsData = textsData.filter((obj: any,index: any) => {
                                    return afterData.includes(index)
                                })
                ...
                // let {class_id}  = d
                // this.props.handle_change_click(d.class_id)
    ...
    componentDidMount(){
      axios.get(`${PATH}/get_scatterData`,{withCredentials:true}).then((res:AxiosResponse<any, any>) =>{
    // axios.get(`${PATH}/3000class_scatter_data.json`,{withCredentials:true}).then((res:AxiosResponse<any, any>) =>{
        // console.log(res.data.data)
        this.setState({
            scatter_data:res.data.data
        })
      })
      this.draw_circle()
    }
...
const mapDispatchToProps = {
    updateMarkTextData,
    changeMenuSelect,
    changeClusteringPlotClass
}

export default connect(null,mapDispatchToProps)(ClusteringPlot);